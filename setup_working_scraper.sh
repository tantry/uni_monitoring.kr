#!/bin/bash
echo "=== Setting up Working Scraper with Chrome ==="

# First, let's check if we can use Chrome directly
echo "1. Testing Chrome access..."
...
# Try installing Selenium without ChromeDriver first
pip install selenium

echo -e "\n3. Creating working scraper without Selenium dependency..."
cat > scrapers/adiga_working.py << 'WORKING_SCRAPER'
"""
...
def test_scraper():
    """Test function"""
    scraper = AdigaWorkingScraper({'url': 'https://adiga.kr'})
...
# Create a simple test and run script
echo -e "\n4. Creating test script..."
cat > test_and_run.sh << 'TEST_RUN'
#!/bin/bash
...
echo -e "\n=== Setup Complete ==="
echo -e "\nYour Chrome is at: /usr/bin/google-chrome-stable"
echo -e "\nIf you want to use Selenium later:"
